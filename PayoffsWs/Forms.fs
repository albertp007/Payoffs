...
module Forms =
  ...
  let ImpliedVolCalculatorFormlet() : Formlet<ImpliedVolQuery> =
    ...
    let t = makeFloatField "0.0" "Time to expiry required" "Time to expiry (years)"
    let k = makeFloatField "0.0" "Strike required" "Strike"
    let optType = Select 0 OptionTypeList |> Enhance.WithTextLabel "Option type"
    ...
    let maxIter = makeIntField "100" "Maximum iteration required" "Max iteration"

    Formlet.Yield (fun s0 r q t k optType p precision maxIter->
      { s0=float s0; r=float r; q=float q; T=float t; K=float k; 
        OptType=optType; price=float p; precision=float precision; 
